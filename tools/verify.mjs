...
// The dependency map's served views (docs/depmap/index.html, the record; docs/depmap/architecture.html, the harness)
// are generated by tools/depmap.mjs and sit outside the page floor by DR-010's one exception (DR-017, revised
// 2026-09-10 and 2026-09-15); section 8 holds each to freshness, no-index and no external load instead.
const MAP_VIEWS = [
  { file: join(DOCS, 'depmap', 'index.html'), scope: 'record', cmd: 'node tools/depmap.mjs view --site' },
  { file: join(DOCS, 'depmap', 'architecture.html'), scope: 'architecture', cmd: 'node tools/depmap.mjs view --site --scope architecture' },
];
const isMapView = (f) => MAP_VIEWS.some((v) => v.file === f);
const htmlFiles = files.filter((f) => f.endsWith('.html') && !isMapView(f));
...
if (existsSync(MANIFEST)) {
  ...
  if (man) {
    ...
    const relAll = [...relPages, ...MAP_VIEWS.filter((v) => existsSync(v.file)).map((v) => posix.relative(DOCS.split('\\').join('/'), v.file.split('\\').join('/')))];
...
// The served map views: generated, committed, and never stale, indexed or loading from outside (DR-010's exception,
// DR-017 revised). They are the two pages outside the page floor, and both render from the one graph.
for (const v of MAP_VIEWS) {
  if (!existsSync(v.file)) { fail(v.file, 1, `the served ${v.scope} view is missing; run ${v.cmd} (DR-017)`); continue; }
  const t = read(v.file);
  let expected = null;
  try { expected = depmap.renderView({ root: ROOT, mode: 'site', scope: v.scope }).page; } catch (e) { fail(v.file, 1, `the served ${v.scope} view cannot be rendered: ${e.message}`); }
  if (expected !== null && expected !== t) fail(v.file, 1, `the served ${v.scope} view is stale; run ${v.cmd}`);
  if (!/<meta\s+name="robots"\s+content="noindex[^"]*"/.test(t)) fail(v.file, 1, `the served ${v.scope} view lacks the no-index directive`);
  const ext = /<(link|script|img|source|use|image)\b[^>]*\b(href|src|xlink:href)\s*=\s*"(https?:)?\/\/[^"]*"/i.exec(t);
  if (ext) fail(v.file, lineOf(t, ext.index), `the served ${v.scope} view loads an external asset: ${ext[0].slice(0, 80)}`);
  // A page-relative link inside a view must resolve, as every other internal link must.
  const hrefRe = /\bhref="(\.\.?\/[^"#]*)(?:#[^"]*)?"/g;
  let hm;
  while ((hm = hrefRe.exec(t))) {
    const target = resolve(dirname(v.file), hm[1]);
    const file = existsSync(target) && statSync(target).isDirectory() ? join(target, 'index.html') : target;
    if (!existsSync(file)) fail(v.file, lineOf(t, hm.index), `link target does not exist: ${hm[1]}`);
  }
}
