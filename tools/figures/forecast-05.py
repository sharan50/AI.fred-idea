"""Figures 5.1 to 5.4 for /05-business/, generated from models/sim_viable.csv.
...
rows = list(csv.DictReader(open(R / 'models/sim_viable.csv')))
CR = 1e7  # one crore
def col(name): return [float(r[name]) for r in rows]
months = [int(r['month']) for r in rows]
S = {k: col(k + '_plan') for k in ('revenue', 'labour', 'tokens', 'eng', 'compliance', 'sales', 'gna', 'onetime', 'total_cost', 'cum_cash')}
...
def frame(series, fmt, unit, marks, accent_key, zero_line=False, height=400, clip='clip', hi_cap=None, exit_note=None, label_gap=0):
...
h, body = frame({'planning line': [v / CR for v in S['cum_cash']], 'conservative line': [v / CR for v in S['cash_cons']]},
 ...
 [(28, 'planning line', 'trough m28, 17.3 cr', False, -10, 24, 'end'),
  (56, 'conservative line', 'trough m56, 49.6 cr', True, -10, -54, 'end')],
 'planning line', zero_line=True, height=400, clip='clip-cc', hi_cap=60, label_gap=64,
 exit_note=('the planning line leaves the frame at m46', 'and reaches 146 crore by m60'))
